...
describe("Placing ships", () => {
   test("Place and check new ships coordinates", () => {
      ...
      expect(ship2.getCoors()).toEqual([
         [5, 2],
         [6, 2],
      ]);

      expect(() => gameboard2.placeShip([9, 9], 2)).toThrow(
         "Ship expands to wrong coordinates"
      );

      gameboard2.placeShip([6, 5], 4);
      gameboard2.placeShip([7, 0], 4, "asdf");

      expect(gameboard2.getShips().type2.ships[0].getCoors()).toEqual([
         [6, 5],
         [6, 6],
         [6, 7],
         [6, 8],
      ]);
      expect(gameboard2.getShips().type2.ships[1].getCoors()).toEqual([
         [7, 0],
         [7, 1],
         [7, 2],
         [7, 3],
      ...
      expect(() => gameboard2.placeShip([8, "hi"], 2)).toThrow(
         "Coordinates should be numbers"
      );
      expect(() => gameboard2.placeShip([0, 7], [])).toThrow(
         "Length should be a number between 2 and 5"
      );
   });
});
...
describe("Get board with ships", () => {
   test("Print board with ships", () => {
      ...
      expect(gameboard.getBoardAndShips()).toEqual([
         ["s", "s", "s", "s", "s", "~", "~", "~", "~", "~"],
         ["~", "s", "~", "~", "~", "~", "~", "~", "~", "s"],
         ["~", "s", "~", "~", "~", "~", "~", "~", "~", "s"],
         ["~", "s", "~", "~", "~", "~", "s", "s", "~", "~"],
...
describe("Prevent exceed maximum number of ships", () => {
   test("Exceed maximum number of 2 length ships", () => {
      ...
      expect(() => gameboard3.placeShip([5, 0], 2)).toThrow(
         "Exceeded number of ships: maximun number for 2 length ships is 4"
      );
   });
});
...
describe("Place new ships", () => {
   test("Place all types of ships", () => {
      ...
      expect(() => gameboard4.placeShip([0, 7], 2)).toThrow(
         "Exceeded number of ships: maximun number for 2 length ships is 4"
      );
      expect(() => gameboard4.placeShip([0, 7], 5, "ver")).toThrow(
         "Exceeded number of ships: maximun number for 5 length ships is 1"
      );
   });
});
...
describe("Place ship over another ship", () => {
   test("Prevent a new ship on the same coordinates", () => {
      ...
      expect(() => gameboard5.placeShip([0, 4], 4)).toThrow(
      ...
      expect(() => gameboard5.placeShip([0, 9], 4, "ver")).toThrow(
         "A new ship cannot be place over another"
      );
   });
});
...
describe("Remove ships", () => {
   test("Place and remove all types of ships", () => {
      ...
      expect(gameboard.removeShip(0, 0)).toBe(
         "Removed ship with the following coordinates: [0, 0], [0, 1], [0, 2], [0, 3], [0, 4]"
      );
      expect(gameboard.getShips().type1.ships.length).toBe(0);

      expect(gameboard.removeShip(1, 0)).toBe(
         "Removed ship with the following coordinates: [1, 0], [1, 1], [1, 2], [1, 3]"
      );
      expect(gameboard.getShips().type2.ships.length).toBe(1);

      // remove ships by not pointing to the coordinates of the first chunk
      expect(gameboard.removeShip(4, 1)).toBe(
         "Removed ship with the following coordinates: [4, 0], [4, 1], [4, 2]"
      );
      expect(gameboard.getShips().type3.ships.length).toBe(2);

      expect(gameboard.removeShip(4, 4)).toBe(
         "Removed ship with the following coordinates: [4, 3], [4, 4]"
      );
      expect(gameboard.getShips().type4.ships.length).toBe(3);
   });
   ...
   test("Remove ship that does not exist", () => {
      ...
      expect(gameboard.removeShip(0, 0)).toBe(
         "There is no ship in [0,0] coordinates"
      );
   });
});
